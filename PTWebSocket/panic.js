...
const serverConfig = JSON.parse(fs.readFileSync('../public/panic_config.json', 'utf8'));
...
wss.on('connection', function connection(ws,req) {
    ...
    ws.on('message', function incoming(message) {
        ...
        // receive a message with a panic code, check the code on the server, send a panic to everyone
        if (message[0] === '15987615'){
            sendAll("Panic!", message[1]);
        }
        console.log(message);
    });
...
function sendAll(message, shop) {
    for (var i=0; i < clients.length; i++) {
        if (clients[i].shop === shop){
            // Send messages to everyone, including the sender
