...
CPusher = function () {
	...
	function showWheelBonusMsg(bonuspercent,bonusamount) {
		...
		$("#wheelbonus").fadeIn();	
		//hide gameframe in mobile 
		MobileGameframeCheck(28000);						
	}
	...
	function showBonusMsg(bonusamount) {
		...
		//hide gameframe in mobile 
		MobileGameframeCheck(6000);				
		
		$("#bonusdiv").html(" + " + bonusamount);
		$("#bonusdiv").fadeIn(),		
		setTimeout(hideBonusMsg, 6000);
	...
	function MobileGameframeCheck(showafter){		
		if(window.mobileAndTabletcheck){
			var gamefrm = document.getElementById("gameframe");
			if (gamefrm != null){	
				$(gamefrm).hide();
				showgameframe = true;
				setTimeout(function(){ if(showgameframe){$(gamefrm).show(); showgameframe = false;}}, showafter);
			}
		}
	}
	...
	function refreshGameCredit(sdata) {		
		...
		try {
			var gamefrm = document.getElementById("gameframe");
			if (gamefrm != null){				
				gamefrm.contentWindow.postMessage(["reloadBalance", "0"], '*');
			
				var gameDoc = gamefrm.contentDocument || gamefrm.contentWindow.document;				
				var gameifrm = gameDoc.getElementById("gameiframe");						
				if (gameifrm != null){					
					gameifrm.contentWindow.postMessage(["reloadBalance", "0"], '*');
				}
			}			
		} catch (err) {}		
	}	
...
{	
	...
	$.ajax({
		...
		success: function (msg) {
			bomba = document.getElementById("gameframe");
			if (bomba != null)
