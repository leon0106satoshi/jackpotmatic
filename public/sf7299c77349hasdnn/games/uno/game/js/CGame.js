function CGame(oData) {
    ...
    this.startGame = function() {
        ...
        this.getFirstHand();
        _oAlertText = new CTLText(s_oStage, CANVAS_WIDTH / 2 + 280, CANVAS_HEIGHT / 2, 200, 150, 50, "center", "#fff", PRIMARY_FONT, 1, 0, 0, TEXT_ALERT_1, true, true, true, false);
        ...
        if (oCard.getFotogram() === "color") {
            ...
            if (oCard.getRank() === 10) {
                ...
                if (_bUNO === true) {
                    setTimeout(function() {
                        if (_bUNO === true) {
                            oParent.drawCards(0, 2, 0, bPenality);
                        };
                        setTimeout(function() {
    ...
    this.onNextTurn = function() {
        if (this.checkGameOver() === true) {} else if (_oDeck.getLength() === 0 && _bEntered === false) {
            _bEntered = true;
                ...
                for (var i = 0; i < _aPlayersHand[iThisTurn].getLength(); i++) {
                    if (_aPlayersHand[iThisTurn].getCardByIndex(i).getSuit() !== 4 && (_aPlayersHand[iThisTurn].getCardByIndex(i).getRank() === oLastCard.getRank() || _aPlayersHand[iThisTurn].getCardByIndex(i).getSuit() === _iCurrentColor)) {
                        if (_aPlayersHand[iThisTurn].getCardByIndex(i).getRank() === 12) {
                            aBestCards.push({
                                oCard: _aPlayersHand[iThisTurn].getCardByIndex(i),
                                iValue: 6
                            });
    ...
    this.drawCards = function(iIndexPlayer, iNumberOfCards, iDelay, bPenality) {
        if (bPenality) {
            _bUNO = false;
            new createjs.Tween.get(_oAlertText.getText()).to({
                alpha: 1
            }, 400).wait(4000).to({
                alpha: 0
            }, 400);
    ...
    this.drawCardsTween = function(iIndexPlayer, iNumberOfCards, iDelay, bPenality) {
        ...
        if (iNumberOfCards === 1) {
            ...
            new createjs.Tween.get(oCard.getSprite()).wait(iDelay).to({
                x: (oHandGlobalPos.x + oCardPos.x) - (oDeckGlobalPos.x),
                y: (oHandGlobalPos.y + oCardPos.y) - (oDeckGlobalPos.y)
    ...
    this.checkHelpDraw = function() {
        setTimeout(function() {
            if (_bCheckDraw === true) {
                s_oStage.addChildAt(_oFinger, s_oStage.numChildren);
            }
        }, 5000);
    };
