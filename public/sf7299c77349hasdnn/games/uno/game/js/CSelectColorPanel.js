function CSelectColorPanel(szType, bPenality) {
    ...
    this._init = function() {
        ...
        var oTitle = new CTLText(_oPanelContainer, -250, -oSprite.height / 2 + 10, 500, 110, 50, "center", "#fff", PRIMARY_FONT, 1, 0, 0, TEXT_SELECT_COLOR, true, true, true, false);
        ...
        _oButRed.addEventListener(ON_MOUSE_UP, function() {
            _oParent.onSelectColor(0), _oColorContainer;
        });
        oSprite = s_oSpriteLibrary.getSprite('but_green');
        _oButGreen = new CGfxButton(-65, -20, oSprite, _oColorContainer);
        _oButGreen.addEventListener(ON_MOUSE_UP, function() {
            _oParent.onSelectColor(1), _oColorContainer;
        });
        oSprite = s_oSpriteLibrary.getSprite('but_blue');
        _oButBlue = new CGfxButton(65, -20, oSprite, _oColorContainer);
        _oButBlue.addEventListener(ON_MOUSE_UP, function() {
            _oParent.onSelectColor(2), _oColorContainer;
        });
        oSprite = s_oSpriteLibrary.getSprite('but_yellow');
        _oButYellow = new CGfxButton(190, -20, oSprite, _oColorContainer);
        _oButYellow.addEventListener(ON_MOUSE_UP, function() {
            _oParent.onSelectColor(3), _oColorContainer;
        });
    };
                ...
                if (bPenality) {
                    setTimeout(function() {
                        if (s_oGame.getbUNO() === true) {
                            s_oGame.drawCards(0, 2, 0, bPenality)
                        } else {
                            s_oGame.onNextTurn()
                        }
                    }, 2000);
