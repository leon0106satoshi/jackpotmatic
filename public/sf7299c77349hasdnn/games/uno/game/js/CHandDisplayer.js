function CHandDisplayer(iOffsetX, iOffsetY, iX, iY, iPlayer, oParentContainer, TEXTNAME) {
    ...
    this.getGlobalPosition = function() {
        ...
        oGlobalPosition = {
            x: _oContainer.x + oCardsInfoX,
            y: _oContainer.y + oCardsInfoY
        };
    ...
    this.getPosNewCard = function() {
        return {
            x: _aCards.length * _iOffsetX,
            y: _aCards.length * _iOffsetY
        };
    };
    ...
    this.centerContainer = function() {
        ...
        if (_iOffsetX !== 0) {
            createjs.Tween.get(_oContainer).to({
                x: CANVAS_WIDTH / 2 - (oHandInfo.width / 2) + (CARD_WIDTH / 2)
            }, 300, createjs.Ease.linear);
        } else {
            createjs.Tween.get(_oContainer).to({
                y: CANVAS_HEIGHT / 2 - (oHandInfo.height / 2) + (CARD_HEIGHT / 1.5) - 40
            }, 300, createjs.Ease.linear);
        }
    };
    ...
    this.organizeHand = function(iParent) {
        ...
        if (_iOffsetX !== 0) {
            for (var i = 0; i < _aCards.length; i++) {
                ...
                createjs.Tween.get(_aCards[i].getSprite()).to({
                    x: iOffsetIncrease
                }, 300, createjs.Ease.linear).call(oParent.centerContainer);
            }
        } else {
            for (var i = 0; i < _aCards.length; i++) {
    ...
    this.onTweenPlayer = function() {
        var oParent = this;
        if (_bThisTurn === true) {
            new createjs.Tween.get(_oPlayerText).to({
                alpha: 0.15
            }, 500).to({
                alpha: 1
            }, 500).call(function() {
                oParent.onTweenPlayer();
            });
        } else {
    ...
    this.checkUno = function() {
        if (_aCards.length === 1) {
            ...
            new createjs.Tween.get(_oCloudUno).to({
                alpha: 1,
                scaleX: 1,
                scaleY: 1
                ...
                s_oStage.removeChild(_oCloudUno);
            });
        }
    };
