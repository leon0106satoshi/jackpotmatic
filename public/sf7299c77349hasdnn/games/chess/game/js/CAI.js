function CAI() {
    ...
    this._buildTree = function() {
        ...
        _oDecisionTree = new CTreeDecision({
            rating: 0,
            moves: [],
            depth: iStartingDepth
        });
                    ...
                    if (_aCurBoard[i][j].color === szEvalPlayerColor) {
                        ...
                        for (var k = 0; k < aSinglePieceMoveList.length; k++) {
                            ...
                            aAllMovesList.push({
                                ...
                                destrow: iDestRow,
                                destcol: iDestCol,
                                sourcetype: _aCurBoard[i][j].type,
                                sourcecolor: _aCurBoard[i][j].color,
                                sourcehistory: _aCurBoard[i][j].history,
                                desttype: _aCurBoard[iDestRow][iDestCol].type,
                                destcolor: _aCurBoard[iDestRow][iDestCol].color,
                                desthistory: _aCurBoard[iDestRow][iDestCol].history
                            });
                        }
                    } else {
                        aOpponentList.push({
    ...
    this._makeMove = function(oMove) {
        ...
        _aCurBoard[oMove.destrow][oMove.destcol] = {
            color: oMove.sourcecolor,
            type: oMove.sourcetype,
            history: oMove.sourcehistory
        };
        _aCurBoard[oMove.sourcerow][oMove.sourcecol] = {
            color: null,
            type: null,
            history: []
        };
    ...
    this._copyBoard = function(aBoard) {
        ...
        for (var i = 0; i < NUM_CELL; i++) {
            ...
            for (var j = 0; j < NUM_CELL; j++) {
                aNewBoard[i][j] = {
                    color: aBoard[i][j].getColor(),
                    type: aBoard[i][j].getType(),
                    history: aBoard[i][j].getPieceHistory()
                };
