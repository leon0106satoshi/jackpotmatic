...
class Functions {
    ...
    public static function count_refunds($count_refund, $shop_id){

        $return = \VanguardLTE\Refund::where(['shop_id' => $shop_id, 'status' => 1])
            ->whereRaw("'".$count_refund."' BETWEEN min_pay AND max_pay")->first();
        if( $return ){
            $sum = floatval($return->percent)/100 * $count_refund;
        } else{
            $sum = 0;
        }
