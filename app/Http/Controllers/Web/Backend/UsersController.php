...
namespace VanguardLTE\Http\Controllers\Web\Backend
{
    ...
    class UsersController extends \VanguardLTE\Http\Controllers\Controller
    {
        ...
        public function store(\VanguardLTE\Http\Requests\User\CreateUserRequest $request)
        {
            ...
            {
                if($request->cashier == 'cashier')
                    return redirect()->route('backend.user.delete', 20)->withErrors([trans('app.choose_shop')]);
            ...
            {
                if($request->cashier == 'cashier')
                    return redirect()->route('backend.user.delete', 20)->withErrors([__('app.blocked_domain_zone', ['zone' => $return['domain']])]);
            ...
            {
                if($request->cashier == 'cashier')
                    return redirect()->route('backend.user.delete', 20)->withErrors([trans('app.max_users', ['max' => $this->max_users])]);
            ...
            if( $request->balance && $request->balance > 0 ) 
            {
                ...
                {
                    if($request->cashier == 'cashier')
                        return redirect()->route('backend.user.delete', 20)->withErrors([trans('app.not_enough_money_in_the_shop', [
                            'name' => $shop->name, 
                ...
                {
                    if($request->cashier == 'cashier')
                        return redirect()->route('backend.user.delete', 20)->withErrors([trans('app.shift_not_opened')]);
            ...
            {
                if($request->cashier == 'cashier')
                    return redirect()->route('backend.user.delete', 20)->withErrors([trans('app.only_1', ['type' => $role->slug])]);
            ...
            if($request->cashier = 'cashier')
                return redirect()->route('backend.user.delete', 20)->withSuccess(trans('app.user_created'));
        ...
        {
            $redirectroute = 'backend.user.list';
            $users = auth()->user()->availableUsers();
            $google2fa = app('pragmarx.google2fa');
            if( count($users) && !in_array($user->id, $users) ) 
            {
            ...
            {
                // var_dump($request->status, $user->status);
        ...
        public function updateBalance(\Illuminate\Http\Request $request)
        {
            ...
            {
                if($request->cashier == 'cashier')
                    return redirect()->route('backend.user.delete', 20)->withErrors($result['message']);
                return redirect()->back()->withErrors($result['message']);
            }
            if($request->cashier == 'cashier')
                return redirect()->route('backend.user.delete', 20)->withSuccess([$result['message']]);
        ...
        public function cashier(\Illuminate\Http\Request $request){
            $statuses = ['' => trans('app.all')] + \VanguardLTE\Support\Enum\UserStatus::lists();
            $roles = \jeremykenedy\LaravelRoles\Models\Role::where('level', '<', auth()->user()->level())->pluck('name', 'id');
            $roles->prepend(trans('app.all'), '0');
            $shop = \VanguardLTE\Shop::find(auth()->user()->shop_id);
            ...
            return view('backend.user.cashier', compact('users', 'statuses', 'roles', 'happyhour', 'activeUsers'));
        }
        ...
        public function delete(\VanguardLTE\User $user, \Illuminate\Http\Request $request)
        {
            // var_dump([trans('app.balance_not_zero')]);
            // exit();
            if( $user->id == auth()->user()->id ) 
            {
                if($request->cashier == 'cashier')
                    return redirect()->route('backend.cashier')->withErrors(trans('app.you_cannot_delete_yourself'));
                return redirect()->route('backend.list')->withErrors(trans('app.you_cannot_delete_yourself'));
            }
            if( $user->balance > 0 ) 
            {
                if($request->cashier == 'cashier')
                    return redirect()->route('backend.cashier')->withErrors(trans('app.balance_not_zero'));
                return redirect()->route('backend.list')->withErrors([trans('app.balance_not_zero')]);
            }
            ...
            {
                if($request->cashier == 'cashier')
                    return redirect()->route('backend.cashier')->withErrors([trans('app.has_users', ['name' => $user->username])]);
                return redirect()->route('backend.list')->withErrors([trans('app.has_users', ['name' => $user->username])]);
            }
            ...
            {
                if($request->cashier == 'cashier')
                    return redirect()->route('backend.cashier')->withErrors([trans('app.has_stats', ['name' => $user->username])]);
                return redirect()->route('backend.list')->withErrors([trans('app.has_stats', ['name' => $user->username])]);
            }
            $user->detachAllRoles();
            ...
            $user->delete();
            if($request->cashier == 'cashier')
                return redirect()->route('backend.cashier')->withSuccess(trans('app.user_deleted'));
            return redirect()->route('backend.list')->withSuccess(trans('app.user_deleted'));
        }
