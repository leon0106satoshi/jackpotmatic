    ...
    class DashboardController extends \VanguardLTE\Http\Controllers\Controller
    {
        ...
        public function index()
        {
            ...
            if( \Auth::user()->hasRole('cashier') ) {
                return redirect()->route('backend.cashier');
            }
        ...
        public function live_stat(\Illuminate\Http\Request $request)
        {
            ...
            // $statgames = \VanguardLTE\StatGame::select(DB::raw('t.*'))
            //     ->from(DB::raw('(SELECT * FROM w_stat_game ORDER BY id DESC) t'))
            //     ->groupBy('t.user_id');
            // $statgames = \VanguardLTE\StatGame::select('t.*')->whereIn('t.shop_id', $availableShops)->distinct('from');
            
            // $statgames = $statgames->whereIn('t.user_id', $users);
            // $statgames = \VanguardLTE\StatGame::select('stat_game.*')->orderBy('id', 'DESC');
            // $statgames = $statgames->get();
            $users = \VanguardLTE\StatGame::select('stat_game.*')->groupBy('user_id')->get();
            // var_dump(count($statgames));
            // exit();
            $final_stat = [];
            foreach($users as $user){
                // var_dump($user->user_id);
                $stat = \VanguardLTE\StatGame::select('stat_game.*')->where('user_id', $user->user_id)->take(1)->orderBy('id', 'DESC')->get()[0];
                // var_dump($user->user_id, $stat->game);
                $final_stat[] = $stat;
                // foreach($statgames as $stat)
                //     if($stat->user->id == $user->user_id){
                //         $final_stat[] = $stat;
                //         break;
                //     }
            }
            // exit();
            $statgames = $final_stat;
            if( auth()->user()->hasPermission('stats.game') && (!$request->type || $request->type && $request->type == 'StatGame') )
            {
                foreach( $statgames as $stat )
                {
                    $username = \VanguardLTE\User::select('users.*')->where('id', $stat->user_id)->get();
                    // var_dump($username);
                    if(count($username))
                    ...
                    $statistics[] = [
                        ...
                        'IN_JPG' => $stat->in_jpg,
                        'Slots' => $stat->slots_bank,
                        'Bonus' => $stat->bonus_bank,
                        'Total_bank' => $stat->total_bank,
                        'Profit' => $stat->in_profit,
            ...
            $statistics = array_slice($statistics, 0, 100);
            return view(\Auth::user()->hasRole(['cashier','distributor']) ? 'backend.stat.live_stat_cashier' : 'backend.stat.live_stat', compact('statistics', 'filter'));
        }
            ...
            if( auth()->user()->hasRole('cashier') ) 
            {
                ...
                if( $count ) 
                {
                    ...
                    $count->update([
                        ...
                        'jpg' => \VanguardLTE\JPG::where('shop_id', $shop_id)->sum('balance'), 
                        'users' => $summ
                    ]);
                }
                ...
                return redirect()->route('backend.cashier')->withSuccess(trans('app.open_shift_started'));
            }
            else
            {
                abort(403);
        ...
        public function transactions(\Illuminate\Http\Request $request)
        {
            ...
            $statistics = \VanguardLTE\Statistic::lockForUpdate()->select([
                'id', 
                'title', 
                'user_id', 
                'payeer_id', 
            ...
            $total_money = \VanguardLTE\Statistic::select('statistics_add.*')->whereNotIn('statistics.system', [
                'invite', 
                'progress', 
                'tournament', 
                'daily_entry', 
            ...
            $money_in = \VanguardLTE\Statistic::select('statistics_add.*')->whereNotIn('statistics.system', [
                'invite', 
                'progress', 
                'tournament', 
                'daily_entry', 
            ...
            $money_out = \VanguardLTE\Statistic::select('statistics_add.*')->whereNotIn('statistics.system', [
                'invite', 
                'progress', 
                'tournament', 
                'daily_entry', 
        ...
        public function ticket_stat(\Illuminate\Http\Request $request){
            
            $users = auth()->user()->availableUsers();
            $tickets = \VanguardLTE\WithdrawTicket::whereIn('player_id', $users);
            if( $request->user != '' ) 
            {
                $tickets=$tickets->where('player_name', 'like', '%'. $request->user.'%');
            }
            if( $request->amount != '' ) 
            {
                $tickets=$tickets->where('ticket_amount', $request->amount);
            }
            if( $request->number != '' ) 
            {
                $tickets->where('id', $request->number);
            }
            if( $request->dates != '' ) 
            {
                $dates = explode(' - ', $request->dates);
                $tickets = $tickets->where('created_at', '>=', $dates[0]);
                $tickets = $tickets->where('created_at', '<=', $dates[1]);
            }
            ...
            $perPage = 50;
            $offset = $page * $perPage - $perPage;
            $tickets = $tickets->offset($offset)->take($perPage)->pluck('id');
            $tickets = \VanguardLTE\WithdrawTicket::whereIn('id', $tickets)->orderBy('id', 'DESC')->get();
            
            return view('backend.stat.tickets', compact('tickets', 'count', 'page', 'perPage', 'offset'));
        }
        ...
        public function securities_block(\Illuminate\Http\Request $request, \VanguardLTE\Security $item)
        {
            if( $item ) 
            {
                ...
                {
                    $item->user->update([
                        'status' => \VanguardLTE\Support\Enum\UserStatus::BANNED, 
                        'remember_token' => null
                    ]);
        ...
        public function banks(\Illuminate\Http\Request $request)
        {
            ...
            if( $savedSortFiled && $savedSortOrder && in_array($savedSortOrder, [
                'asc', 
                'desc'
            ]) ) 
            {
                if( in_array($savedSortFiled, [
                    'slots', 
                    'little', 
                    'table_bank', 
                    'fish', 
