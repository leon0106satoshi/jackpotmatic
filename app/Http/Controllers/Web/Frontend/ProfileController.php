...
namespace VanguardLTE\Http\Controllers\Web\Frontend
{
    ...
    class ProfileController extends \VanguardLTE\Http\Controllers\Controller
    {
        ...
        public function bonus(\Illuminate\Http\Request $request){
            $data = [
                'bonus' => \VanguardLTE\User::find(auth()->user()->id)->count_refunds
            ];
        ...
        public function ajax(\Illuminate\Http\Request $request)
        {
            ...
            if( $user ) 
            {
                ...
                $data['profile']['count_refunds'] = number_format(auth()->user()->count_refunds, 2, '.', '');
                $wager_tooltip = '';
        ...
        public function vltcash(\Illuminate\Http\Request $request){
            $user = \VanguardLTE\User::find($request->userhash);
            $shop = \VanguardLTE\Shop::find($user->shop_id);
            $frontend = $shop->frontend;
            return view('frontend.' . $frontend . '.partials.vltcash', compact('user', 'shop'));
        }
    }
