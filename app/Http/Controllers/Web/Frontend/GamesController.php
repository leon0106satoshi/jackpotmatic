    ...
    class GamesController extends \VanguardLTE\Http\Controllers\Controller
    {
        ...
        public function index(\Illuminate\Http\Request $request, $category1 = '', $category2 = '')
        {
            ...
            foreach($games as $index => $game){
                $game_category = \VanguardLTE\GameCategory::where('game_id', $game->original_id)->get();
                $category_id_for_game = $game_category[0]->category_id;
                $cat_for_game = \VanguardLTE\Category::find($category_id_for_game);
                $games[$index]->mobile = 1;
                $games[$index]->brand = $cat_for_game->href;
                $games[$index]->category = 'all,'.$cat_for_game->title;
                $games[$index]->category_plain = $cat_for_game->href;
                $games[$index]->new = 1;
                $games[$index]->tag = $game->name;
                $games[$index]->type = 'slot';
            }
            foreach($categories as $index => $category){
                $categories[$index]->tag = $category->title;
                $categories[$index]->name = $category->title;
            }
            // var_dump('category1', $categories);
            // exit();
            return view('frontend.' . $frontend . '.games.index', compact('games', 'category1', 'cat1', 'categories', 'currentSliderNum', 'title', 'body', 'keywords', 'description', 'jpgs', 'shop', 'devices', 'tournament', 'is_game_page', 'jpgSum'));
        }
        ...
        public function check_redirect(\Illuminate\Http\Request $request, $category1, $error = '')
        {
            ...
            if( $category1 == '' ) 
            {
                if( $currentCategory = $request->cookie('currentCategory' . auth()->user()->id) ) 
                {
        ...
        public function do_redirect(\Illuminate\Http\Request $request, $option, $error)
        {
            ...
            {
                return redirect()->route('frontend.game.list.category', $option)->withSuccess(\Session::get('success'));
            }
            return redirect()->route('frontend.game.list.category', $option);
        }
        ...
        public function search(\Illuminate\Http\Request $request)
        {
            ...
            if( count($games) ) 
            {
                foreach( $games as $game ) 
                {
                    $returns .= view('frontend.euroslots.partials.game', compact('game', 'currency'));
                }
            }
            return json_encode([
                'success' => true, 
        ...
        public function go(\Illuminate\Http\Request $request, $game)
        {
            ...
            $slot = $object ? new $object($game->name, auth()->user()->id) : '';
            $is_api = false;
        ...
        public function merkur(\Illuminate\Http\Request $request, $category, $game, $command)
        {
            if( \Illuminate\Support\Facades\Auth::check() && !auth()->user()->hasRole('user') )
            {
                echo '{"responseEvent":"error","responseType":"start","serverResponse":"Wrong User"}';
            ...
            \VanguardLTE\Subsession::where('id', '!=', $subssession->id)->where('user_id', auth()->user()->id)->update(['active' => 0]);
            $object = '\VanguardLTE\Games\\' . $category . '\\' . $game . '\Server';
            $server = new $object();
            return $server->get($request, $game, $command);
        }
