...
namespace VanguardLTE
{
    class User extends \Illuminate\Foundation\Auth\User implements \Tymon\JWTAuth\Contracts\JWTSubject
    {
        ...
        {
            $ticket = WithdrawTicket::where(['player_id'=>$this->id])->latest()->first();
        ...
        public function availableUsers()
        {
            ...
            if( $this->hasRole(['agent']) ) 
            {
                ...
                $childs = User::where('parent_id', $this->id)->get()->pluck('id');
                $childs[] = $this->id;
                $child_shops = Shop::where('user_id', $childs)->get()->pluck('id');
                $child_shops = $child_shops->toArray() + $this->shops_array(true);
                $other = User::where('role_id', '<=', 3)->whereIn('shop_id', $child_shops)->get();
            ...
            // if( !$this->hasRole(['admin']) && !$this->hasRole(['agent']) ) {
            //     $users = $users->where('parent_id', $this->id);
            // }
        ...
        public function shops($onlyId = false)
        {
            ...
            if( count($shops) ) 
            {
                if( $onlyId ) 
                {
                    return Shop::whereIn('id', $shops)->orWhere('id', $this->shop_id)->orWhere('user_id', $this->id)->pluck('id');
                }
                else
                {
                    return Shop::whereIn('id', $shops)->orWhere('id', $this->shop_id)->orWhere('user_id', $this->id)->pluck('name', 'id');
                }
            }
            else
            {
        ...
        public function addBalance($type, $summ, $payeer = false, $refund = true, $system = 'handpay', $update_level = true, $model = false)
        {
            ...
            if( $this->hasRole('user') )
            {
                ...
                // {
                //     // var_dump('!!!4');
                //     $this->update(['count_refunds' => $this->count_refunds + (($summ * $refund) / 100)]);
                // }
                else
                {
                    // var_dump('!!!5 '.Lib\Functions::count_refunds($summ, $this->shop_id));
                    $this->update(['count_refunds' => $this->count_refunds + Lib\Functions::count_refunds($summ, $this->shop_id)]);
                }
            }
            // exit();
            if( $payeer->hasRole('agent') && $this->hasRole('distributor') || $payeer->hasRole('distributor') && $this->hasRole('manager') ) 
