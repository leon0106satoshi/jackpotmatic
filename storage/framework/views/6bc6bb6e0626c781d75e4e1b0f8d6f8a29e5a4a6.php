...
<body>
        ...
        <div id="jackpot-bar">
            ...
            <script>
                function updateJackpots(){
                    jp1.SetJPAmounts(String(Number("<?php echo e($jpgs[1]->balance); ?>").toFixed(2)).split('.').join(''));
                    jp2.SetJPAmounts(String(Number("<?php echo e($jpgs[2]->balance); ?>").toFixed(2)).split('.').join(''));
                    jp3.SetJPAmounts(String(Number("<?php echo e($jpgs[3]->balance); ?>").toFixed(2)).split('.').join(''));
                    ...
                    function reset(){
                        $.ajax({
                            ...
                            success: function (data) {
                                ...
                                jp1.SetJPServerAmount(String(Number("<?php echo e($jpgs[0]->balance); ?>").toFixed(2)).split('.').join(''));
                                jp2.SetJPServerAmount(String(Number("<?php echo e($jpgs[1]->balance); ?>").toFixed(2)).split('.').join(''));
                                jp3.SetJPServerAmount(String(Number("<?php echo e($jpgs[2]->balance); ?>").toFixed(2)).split('.').join(''));
                                jp1.resume();
                                jp2.resume();
                                jp3.resume(); 
                            },
    ...
    $(document).ready(function(){
        ...
        if(is.safari() && top.window.document.getElementById('frame')){
            top.window.location.href="<?php echo e(route('frontend.game.list.category', 'pragmatic')); ?>";
            top.window.document.getElementById('frame').remove()
        }
        json_games1 = <?php echo json_encode($games);; ?>;
        json_categories1 = <?php echo json_encode($categories);; ?>;
        username1 ="<?php echo e(Auth::user()->username); ?>";
        balance1 = <?php echo e(auth()->user()->balance); ?>;
        count_refunds = <?php echo e(auth()->user()->count_refunds); ?>;
        console.log('count_refunds=' + count_refunds);
        jp_bronze1 = String(Number("<?php echo e($jpgs[1]->balance); ?>").toFixed(2));
        jp_silver1 = String(Number("<?php echo e($jpgs[2]->balance); ?>").toFixed(2));
        jp_gold1 = String(Number("<?php echo e($jpgs[3]->balance); ?>").toFixed(2));
        var mobilegametab = undefined;
        OpenGameDiv();

        function getGameUrl(gameName){
            gameRoute = "<?php echo e(route('frontend.game.go', 'abcd')); ?>?api_exit=/";
            gameRoute = gameRoute.replace('abcd', gameName);
            return gameRoute;
        }

        function getLogoutUrl(){
            return "<?php echo e(route('frontend.auth.logout')); ?>";
        }
        ...
        function moveRect() {
            if (window.history && window.history.pushState) {
                window.history.pushState('forward', null, '<?php echo e(config('app.url')); ?>game/ZodiacWheel1?api_exit=/');
                window.location.href = "<?php echo e(config('app.url')); ?>game/ZodiacWheel1?api_exit=/";
            }
        }


        let socket = new WebSocket("wss://<?php echo e(config('app.hostname')); ?>:9999/shop=<?php echo e(auth()->user()->shop_id); ?>");

        socket.onopen = function (e) {
            console.log("[open] Connection established");
            console.log("Sending data to the server");
            socket.send("panic WS active <?php echo e(auth()->user()->shop_id); ?>");
        };
...
<?php echo $__env->make('frontend.euroslots.layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH W:\domains\vladimir.com\resources\views/frontend/euroslots/games/index.blade.php ENDPATH**/ ?>